...
function startertheme_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'startertheme' ) );
		if ( $tags_list ) {
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'startertheme' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
		echo '</span>';
	}
	...
	edit_post_link(
		sprintf(
			...
			esc_html__( 'Edit %s', 'startertheme' ),
...
function startertheme_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'startertheme_categories' ) ) ) {
		...
		set_transient( 'startertheme_categories', $all_the_cool_cats );
	}
